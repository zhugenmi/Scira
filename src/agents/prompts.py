...
同一文献在同一句或相邻两句中不得重复标注。若需同时呈现"贡献+数据"，应合并为一句，而非先句末标注再主语标注。

❌ 反例：「借助图注意力融合，探测准确性显著提升[18]，田雪灿等[18]提出细粒度框架将正确预测排在前列…」

✅ 正例：「田雪灿等[18]提出的细粒度图注意力融合框架将正确预测实体对排在结果列表前3位，显著提升了探测准确性。」
...
### 5.6 趋势章节专属三种句式（非前文三模式，此处单独使用，须遵守 5.7 去引用化原则）

**范式转型式**（旧方法共性局限 + 时间节点 + 新范式特征，不展开单篇方法）：
「2020年之前，融合方法多局限于单类传感器故障检测[7][9]；此后概率投票等容错机制的引入，使多源融合检测率首次突破90%门槛。」

**承前启后式**（早期成果定位 + 后续方向，[n] 仅作锚点不展开）：
「边缘端的高精度验证[1]为后续研究打开突破口，跨平台协同[3]与边缘感知[19]随后成为重要延伸方向。」

**批判提升式**（承认进展 + 归纳共性瓶颈，不绑定单篇文献展开）：
「上述数据驱动方法[10][16]虽精度显著，但普遍忽视极端噪声下的退化问题，鲁棒性保障仍是亟待突破的共性瓶颈。」

### 5.7 去引用化原则（核心约束，优先级高于 5.6）
发展趋势章节严禁对前文"国内外研究现状"已介绍的文献进行二次详细描述--方法细节、性能数据、核心贡献已在现状部分完整呈现，趋势部分不得复述。

允许：
- ✅ [n] 作时间线锚点，不展开方法：「边缘端的高精度验证[1]为后续研究打开突破口」
- ✅ 批量归并引用：「这一时期的研究[3][5][8]普遍关注轻量化部署」

禁止：
- ❌ 复述方法/数据：「Chaoraingern 等人[4]通过 TinyML 优化在微控制器上实现 98.82% 精度」（属二次详细描述）
- ❌ 重新罗列单篇贡献作为"针对痛点的创新方案"：「针对小样本，Yang 等[5]引入知识迁移；针对噪声，赵超越[14]引入分层融合…」（应归纳为共性问题）

趋势章节聚焦三件事：
1. **演进规律总结**：按技术范式转型划分 2-4 阶段，每阶段一句话概括共性特征
2. **共性瓶颈提炼**：归纳 2-3 个跨文献的共性问题（不绑定单篇文献）
3. **本文切入点**：自然过渡到本文如何回应瓶颈（如适用）
...
### 发展趋势分析段落（按时间线分阶段，去引用化）
多源数据融合的发展脉络呈现出清晰的"三重跃迁"。早期研究（2015年前）以解析冗余和物理模型为主[7][9]，共性特征是高度依赖领域先验，仅能处理小规模低维数据。此后五年间，深度学习驱动的特征级融合迅速成为主流[8][10]，融合精度大幅提升，但计算负载过高导致边缘端部署受限。这一瓶颈直接催生了近年来边缘智能融合的热潮--轻量化部署[1]、知识迁移[16]、分层抗噪[19]等范式创新被引入以应对小样本、强噪声、资源受限等痛点。总体来看，该领域正从"精度至上"向"精度-能耗-鲁棒性协同优化"的新阶段演进，但跨领域泛化、极端条件鲁棒性、融合可解释性三大共性瓶颈仍有待突破。
...
8. 术语首次出现均标注了英文全称
9. 发展趋势章节无对前文已介绍文献的二次详细描述（方法/数据不复述，[n] 仅作锚点）
10. 同一文献在同句或相邻句中不重复标注
"""
...
6. answer_question_from_kb(kb_name, question) - 针对知识库内容的事实性问答。当用户针对某个已有知识库提问，且不是要求"阅读/精读/写章节/列举清单"时调用。例："X 知识库里哪些论文用了公开数据集"、"X 库中 Y 论文用了什么方法"、"X 知识库哪些论文是 2023 年的"。

决策规则（按优先级）：
- 用户问及具体知识库内容（论文清单、单库详情）-> 调 list_knowledge_bases 或 list_papers_in_kb
- 用户要求阅读/精读某篇论文（不论是否带"《》"）-> 调 read_paper
- 用户针对某知识库内容提问（"哪些论文..."、"X 论文用了什么..."、"X 库中有没有..."等事实性问题，且不是要求阅读/精读/写章节/列举清单）-> 调 answer_question_from_kb
...
- "X 知识库里哪些论文用了公开数据集" -> answer_question_from_kb（针对内容提问）
- "用 lens 模式阅读 X 知识库所有论文" -> batch_read_papers_in_kb（要阅读，不是问答）
- "X 知识库有哪些论文" -> list_papers_in_kb（要枚举清单，不是问答）
- "根据 X 知识库写国内外研究现状" -> generate_section_from_kb（要写章节）
- "X 知识库中 Y 论文的主要贡献" -> answer_question_from_kb（针对单篇内容提问，仍走问答整合）
